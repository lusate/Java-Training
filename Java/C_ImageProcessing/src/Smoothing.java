...
public class Smoothing {//멤버변수들
	...
	Smoothing(String U_InputFile_Path, String U_OutputFile_Path){
			...
			for(row = 1; row <= width - 2; row++) {
				...
				for (i=-1; i<=1; i++) {
					for(j=-1; j<=1; j++) {
						...
						color = new Color (SourceImage.getRGB(row + i, column + j));
						...
						//calculate sum of 9 neighbors
						red_sum = red_sum + blue;
						green_sum = green_sum + blue;
